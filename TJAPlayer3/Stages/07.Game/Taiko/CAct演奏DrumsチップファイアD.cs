        ...
        public virtual void Start( int nLane, E判定 judge, int player )
		{
            for (int j = 0; j < 3 * 4; j++)
            {
			    ...
			    //for( int n = 0; n < 1; n++ )
			    {
	    			...
	    			this.st状態[ j ].ct進行 = new CCounter( 0, 6, 30, TJAPlayer3.Timer );
                    ...
                    if( this.st状態[ i ].b使用中 )
                    {
				        if( !this.st状態[ i ].ct進行.b停止中 )
				        {
					        ...
					        if( TJAPlayer3.Tx.Effects_Hit_Explosion != null && !TJAPlayer3.IsPerformingCalibration )
					        {
                                ...
                                int nX = ( TJAPlayer3.Skin.Game_Lane_Field_X[ this.st状態[ i ].nPlayer ] ) - ( (TJAPlayer3.Tx.Effects_Hit_Explosion.sz画像サイズ.Width / 8 ) / 2 );
                                int nY = ( TJAPlayer3.Skin.Game_Lane_Field_Y[ this.st状態[ i ].nPlayer ] ) + (195/2) - ( (TJAPlayer3.Tx.Effects_Hit_Explosion.sz画像サイズ.Height / 4 ) / 2 );
                                ...
                                switch( st状態[ i ].judge )
                                {
                                    ...
                                    case E判定.Auto:
                                        if (!this.st状態_大[i].ct進行.b停止中 && TJAPlayer3.Tx.Effects_Hit_Explosion_Big != null && this.st状態_大[i].nIsBig == 1)  
                                                TJAPlayer3.Tx.Effects_Hit_Explosion.t2D描画(TJAPlayer3.app.Device, nX, nY, new Rectangle(this.st状態[i].ct進行.n現在の値 * 255, n + 255 * 2, 255, 255));
                                        else
                                            TJAPlayer3.Tx.Effects_Hit_Explosion.t2D描画(TJAPlayer3.app.Device, nX, nY, new Rectangle(this.st状態[i].ct進行.n現在の値 * 255, n, 255, 255));
                                        break;                                    
                                    case E判定.Good:
                                        if (!this.st状態_大[i].ct進行.b停止中 && TJAPlayer3.Tx.Effects_Hit_Explosion_Big != null && this.st状態_大[i].nIsBig == 1)
                                            TJAPlayer3.Tx.Effects_Hit_Explosion.t2D描画( TJAPlayer3.app.Device, nX, nY, new Rectangle( this.st状態[ i ].ct進行.n現在の値 * 255, n + 255 * 3, 255, 255) );
                                        else
                                            TJAPlayer3.Tx.Effects_Hit_Explosion.t2D描画(TJAPlayer3.app.Device, nX, nY, new Rectangle(this.st状態[i].ct進行.n現在の値 * 255, n + 255, 255, 255));
                                        break;
                                    case E判定.Miss:
                                    case E判定.Bad:
                                        break;
