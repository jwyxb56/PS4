    ...
    internal class CStage演奏ドラム画面 : CStage演奏画面共通
	{
		...
		public override int On進行描画()
		{
			...
			if( !base.b活性化してない )
			{
                ...
                if (!TJAPlayer3.ConfigIni.bNoInfo && TJAPlayer3.ConfigIni.eGameMode != EGame.特訓モード) this.t進行描画_ゲージ();
		...
		{
			TJAPlayer3.ConfigIni.n譜面スクロール速度.Drums = Math.Max( TJAPlayer3.ConfigIni.n譜面スクロール速度.Drums - 1, 1 );
		}
		...
		protected override void t進行描画_チップ_Taiko( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer )
        {
            ...
            if( pChip.b可視 )
            {
                if (!pChip.bHit)
                {
                    ...
                    {
                        case 0:
                            x += ( TJAPlayer3.Skin.Game_Lane_Field_X[ nPlayer ] ) + 100;
                            break;
                        case 1:
                            x = ( TJAPlayer3.Skin.Game_Lane_Field_X[ nPlayer ]) + 100;
                            y = TJAPlayer3.Skin.Game_Lane_Field_Y[ nPlayer ] - xTemp + 100;
                            break;
                        case 2:
                            x = ( TJAPlayer3.Skin.Game_Lane_Field_X[ nPlayer ] + 3 ) + 100;
                            y = TJAPlayer3.Skin.Game_Lane_Field_Y[ nPlayer ] + xTemp + 100;
                            break;
                        case 3:
                            x += ( TJAPlayer3.Skin.Game_Lane_Field_X[ nPlayer ]) + 100;
                        ...
                        case 4:
                            ...
                            break;
                        case 5:
                            x = ( TJAPlayer3.Skin.Game_Lane_Field_X[ nPlayer ] + 10 ) - xTemp + 100;
                            break;
                        case 6:
                            x = ( TJAPlayer3.Skin.Game_Lane_Field_X[ nPlayer ] ) - xTemp + 100;
                            y = TJAPlayer3.Skin.Game_Lane_Field_Y[ nPlayer ] - xTemp + 100;
                            break;
                        case 7:
                            x = ( TJAPlayer3.Skin.Game_Lane_Field_X[ nPlayer ] ) - xTemp + 100;
                    ...
                    if (2200 > x)
                    {
                        if( TJAPlayer3.Tx.Notes != null )
                        {
                            ...
                            x = ( x ) - ( ( int ) ( ( 195.0 * pChip.dbチップサイズ倍率 ) / 1 ) );
                            ...
                            switch ( pChip.nチャンネル番号 )
                            {
                                case 0x11:
                                    ...
                                    {
                                        TJAPlayer3.Tx.Notes.t2D描画( device, x, y , new Rectangle( 195, num9, 195, 195) );
                                    }
                                ...
                                case 0x12:
                                    if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                                    {
                                        TJAPlayer3.Tx.Notes.t2D描画( device, x, y , new Rectangle( 390, num9, 195, 195) );
                                    }
                                ...
                                case 0x13:
                                    if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                                    {
                                        TJAPlayer3.Tx.Notes.t2D描画( device, x, y , new Rectangle( 585, num9, 195, 195) );
                                    }
                                ...
                                case 0x14:
                                    if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                                    {
                                        TJAPlayer3.Tx.Notes.t2D描画( device, x, y, new Rectangle( 780, num9, 195, 195) );
                                    }
                                ...
                                case 0x1A:
                                    if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                                    {
                                        ...
                                        TJAPlayer3.Tx.Notes.t2D描画( device, x, y, new Rectangle( 1690, num9, 195, 195 ) );
                                    }
                                ...
                                case 0x1B:
                                    if( TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON )
                                    {
                                        ...
                                        TJAPlayer3.Tx.Notes.t2D描画( device, x, y , new Rectangle( 1820, num9, 195, 195) );
                                    }
		...
		protected override void t進行描画_チップ_Taiko連打( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer )
        {
            ...
            if (pChip.b可視)
            {
                ...
                int x = TJAPlayer3.Skin.Game_Lane_Field_X[nPlayer] - (195 / 2) + pChip.nバーからの距離dot.Taiko + 10 + 100;
                int x末端 = TJAPlayer3.Skin.Game_Lane_Field_X[nPlayer] - (195 / 2) + pChip.nバーからのノーツ末端距離dot.Taiko + 10 + 100;
                int y = TJAPlayer3.Skin.Game_Lane_Field_Y[nPlayer];
                ...
                if ((2000 > x))
                {
                    if (TJAPlayer3.Tx.Notes != null)
                    {
                        ...
                        if (TJAPlayer3.Skin.Game_Notes_Anime)
                        {
                            if (this.actCombo.n現在のコンボ数[nPlayer] >= 100 && ctChipAnimeLag[nPlayer].b終了値に達した)
                            {
                            ...
                            else if (this.actCombo.n現在のコンボ数[nPlayer] >= 100 && !ctChipAnimeLag[nPlayer].b終了値に達した)
                            {
                                //num9 = base.n現在の音符の顔番号 != 0 ? base.n現在の音符の顔番号 * 130 : 0;
                                if ((int)ctChipAnime[nPlayer].db現在の値 == 1 || (int)ctChipAnime[nPlayer].db現在の値 == 3)
                                {
                        ...
                        if (pChip.nチャンネル番号 == 0x15) //連打(小)
                        {
                            ...
                            if (TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON)
                            {
                                ...
                                TJAPlayer3.Tx.Notes.t2D描画(TJAPlayer3.app.Device, x + 64 + 175 + 20, y, new Rectangle(1170, 0, 128, 195));
                                TJAPlayer3.Tx.Notes.vc拡大縮小倍率.X = 1.0f;
                                TJAPlayer3.Tx.Notes.t2D描画(TJAPlayer3.app.Device, x末端 + f末端ノーツのテクスチャ位置調整 + 175, y, 0, new Rectangle(1364, num9, 195, 195));
                                if (TJAPlayer3.Skin.Game_RollColorMode == CSkin.RollColorMode.All)
                                    TJAPlayer3.Tx.Notes.color4 = effectedColor;
                                else
                                    TJAPlayer3.Tx.Notes.color4 = normalColor;

                                TJAPlayer3.Tx.Notes.t2D描画(TJAPlayer3.app.Device, x + 175, y, 0, new Rectangle(975, num9, 195, 195));
                                TJAPlayer3.Tx.Notes.color4 = normalColor;
                                #endregion
                            }
                            if (TJAPlayer3.Tx.SENotes != null)
                        ...
                        if (pChip.nチャンネル番号 == 0x16)
                        {
                            ...
                            if (TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON)
                            {
                                ...
                                TJAPlayer3.Tx.Notes.t2D描画(TJAPlayer3.app.Device, x + 64 + 175, y, new Rectangle(1753, 0, 128, 195));

                                TJAPlayer3.Tx.Notes.vc拡大縮小倍率.X = 1.0f;
                                TJAPlayer3.Tx.Notes.t2D描画(TJAPlayer3.app.Device, x末端 + f末端ノーツのテクスチャ位置調整 + 175, y, 0, new Rectangle(1950, num9, 195, 195));
                                if (TJAPlayer3.Skin.Game_RollColorMode == CSkin.RollColorMode.All)
                                    TJAPlayer3.Tx.Notes.color4 = effectedColor;
                                else
                                    TJAPlayer3.Tx.Notes.color4 = normalColor;

                                TJAPlayer3.Tx.Notes.t2D描画(TJAPlayer3.app.Device, x + 175, y, new Rectangle(1560, num9, 195, 195));
                                TJAPlayer3.Tx.Notes.color4 = normalColor;
                                #endregion
                            }
                            if (TJAPlayer3.Tx.SENotes != null)
                        ...
                        if (pChip.nチャンネル番号 == 0x17)
                        {
                            if (pChip.bShow)
                            {
                                ...
                                if (TJAPlayer3.ConfigIni.eSTEALTH != Eステルスモード.DORON)
                                    TJAPlayer3.Tx.Notes.t2D描画(TJAPlayer3.app.Device, x + 175, y, new Rectangle(2145, num9, 390, 195));
        ...
        protected override void t進行描画_チップ_小節線( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer )
		{
			...
			if ( ( pChip.b可視 ) && (TJAPlayer3.Tx.Bar != null ) )
			{
                if( x >= 0 )
                {
                    ...
                    mat *= Matrix.RotationZ(C変換.DegreeToRadian(-(135.0f * (float)pChip.dbSCROLL_Y)));
                    mat *= Matrix.Translation((float)(x - 960.0f) - 2.25f, -(y - 528.0f + 97.5f), 0f);
