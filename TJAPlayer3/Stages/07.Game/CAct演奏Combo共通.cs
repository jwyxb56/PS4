    ...
    protected float[,] nコンボ拡大率_座標_1000combo = new float[,]{
        ...
        private void tコンボ表示_太鼓( int nCombo値, int nジャンプインデックス, int nPlayer )
		{
            ...
            for ( int i = 0; i < n桁数; i++ )
            {
				...
				var txTaiko_Combo2 = TJAPlayer3.Tx.Taiko_Combo[2];
				var txTaiko_Combo3 = TJAPlayer3.Tx.Taiko_Combo[3];
				...
				if (txTaiko_Combo2 != null)
				{
					txTaiko_Combo2.Opacity = 255;
				}
				if (txTaiko_Combo2 != null)
				{
					txTaiko_Combo2.Opacity = 255;
				}
				if (txTaiko_Combo3 != null)
				{
					txTaiko_Combo3.Opacity = 255;
				}
				...
				else if (n桁数 == 2)
				{
					...
					{
						var yScalling = ComboScale[this.ctコンボ加算[nPlayer].n現在の値];
						txTaiko_Combo0.vc拡大縮小倍率.Y = TJAPlayer3.Skin.Game_Taiko_Combo_Scale[1] + yScalling;
						txTaiko_Combo0.vc拡大縮小倍率.X = TJAPlayer3.Skin.Game_Taiko_Combo_Scale[1];
						txTaiko_Combo0.t2D拡大率考慮下中心基準描画(TJAPlayer3.app.Device, rightX - TJAPlayer3.Skin.Game_Taiko_Combo_Padding[0] * i, TJAPlayer3.Skin.Game_Taiko_Combo_Y[nPlayer], new Rectangle(n位の数[i] * TJAPlayer3.Skin.Game_Taiko_Combo_Size[0], 0, TJAPlayer3.Skin.Game_Taiko_Combo_Size[0], TJAPlayer3.Skin.Game_Taiko_Combo_Size[1]));
					}
				}
				else if (n桁数 == 4)
				{
					...
					{
						var yScalling = ComboScale[this.ctコンボ加算[nPlayer].n現在の値];
						txTaiko_Combo3.vc拡大縮小倍率.Y = TJAPlayer3.Skin.Game_Taiko_Combo_Scale1[1] + yScalling;
						txTaiko_Combo3.vc拡大縮小倍率.X = TJAPlayer3.Skin.Game_Taiko_Combo_Scale1[1];
						txTaiko_Combo3.t2D拡大率考慮下中心基準描画(TJAPlayer3.app.Device, rightX - TJAPlayer3.Skin.Game_Taiko_Combo_Padding_SPE[0] * i, TJAPlayer3.Skin.Game_Taiko_Combo_Y[nPlayer], new Rectangle(n位の数[i] * TJAPlayer3.Skin.Game_Taiko_Combo_Size[0], 0, TJAPlayer3.Skin.Game_Taiko_Combo_Size[0], TJAPlayer3.Skin.Game_Taiko_Combo_Size[1]));
					}
				}
				else
                {
		...
		public override void On活性化()
		{
			...
			for( int i = 0; i < 4; i++ )
			{
                ...
                this.ctコンボ加算[ i ] = new CCounter( 0, 12, 16, TJAPlayer3.Timer );
			}
