    ...
    internal class CStage選曲 : CStage
    {
		...
		public override void OnManagedリソースの作成()
		{
			if( !base.b活性化してない )
			{
				...
				this.ct背景スクロール用タイマー = new CCounter(0, 1920, 30, TJAPlayer3.Timer);
...
//Debug.WriteLine( "パンくず=" + this.r現在選択中の曲.strBreadcrumbs );
				...
				if(this.act難易度選択画面.bIsDifficltSelect)
                {
					...
					if (genreBack != null)
					{
						genreBack.t2D中心基準描画(TJAPlayer3.app.Device, 960, 540);
					}
					#region nanido
					if (TJAPlayer3.Tx.SongSelect_Frame_Score2 != null)
					{
						...
						if (this.act難易度選択画面.n現在の選択行 >= 0)
							...
							TJAPlayer3.Tx.SongSelect_Score_Select?.t2D下中央基準描画(TJAPlayer3.app.Device, 702 + (this.act難易度選択画面.n現在の選択行 * 132), 441);
					...
					if (TJAPlayer3.Tx.SongSelect_Level != null)
					{
						for (int i = 0; i < (int)Difficulty.Edit + 1; i++)
						{
							for (int n = 0; n < TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[i]; n++)
							{
								...
								if (i == 4 && this.act難易度選択画面.n現在の選択行 == 4)
								{
									//数字
									TJAPlayer3.Tx.SongSelect_Level.t2D下中央基準描画(TJAPlayer3.app.Device, 745 + (3 * 216), TJAPlayer3.Skin.SongSelect_Overall_Y + 567, new Rectangle(35 * TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[i], 87, 35, 45));
									//星
									TJAPlayer3.Tx.SongSelect_Level.t2D下中央基準描画(TJAPlayer3.app.Device, 669 + (3 * 216) + (n * 15), TJAPlayer3.Skin.SongSelect_Overall_Y + 597, new Rectangle(0, 45, 35, 45));
								}
								if (i != 4 && this.act難易度選択画面.n現在の選択行 != 4)
								{
									//数字
									TJAPlayer3.Tx.SongSelect_Level.t2D下中央基準描画(TJAPlayer3.app.Device, 745 + (i * 216), TJAPlayer3.Skin.SongSelect_Overall_Y + 567, new Rectangle(35 * TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[i], 87, 35, 45));
									//星
									TJAPlayer3.Tx.SongSelect_Level.t2D下中央基準描画(TJAPlayer3.app.Device, 669 + (i * 216) + (n * 15), TJAPlayer3.Skin.SongSelect_Overall_Y + 597, new Rectangle(0, 45, 35, 45));
									if (TJAPlayer3.Tx.SongSelect_Branch_Text != null && TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.b譜面分岐[i])
										TJAPlayer3.Tx.SongSelect_Branch_Text.t2D描画(TJAPlayer3.app.Device, 471 + (i * 216), TJAPlayer3.Skin.SongSelect_Overall_Y + 161);
				...
				if ( base.eフェーズID == CStage.Eフェーズ.共通_通常状態 )
				{
					...
					else if (!this.actSortSongs.bIsActivePopupMenu && !this.actQuickConfig.bIsActivePopupMenu && TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Dan)
					{
						#region 難易度選択画面
						if (TJAPlayer3.Pad.b押された(E楽器パート.DRUMS, Eパッド.RBlue))
						{
							this.act難易度選択画面.t次に移動();
							TJAPlayer3.Skin.sound変更音.t再生する();
						}
						else if (TJAPlayer3.Pad.b押された(E楽器パート.DRUMS, Eパッド.LBlue))
						{
							this.act難易度選択画面.t前に移動();
							TJAPlayer3.Skin.sound変更音.t再生する();
						}
						else if (TJAPlayer3.Pad.b押されたDGB(Eパッド.Decide) || (TJAPlayer3.ConfigIni.bEnterがキー割り当てのどこにも使用されていない && TJAPlayer3.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.Return)) || (TJAPlayer3.Pad.b押されたDGB(Eパッド.LRed) || TJAPlayer3.Pad.b押されたDGB(Eパッド.RRed)))
						{
							if (this.act難易度選択画面.n現在の選択行 >= 0)
							{
								if (TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[this.act難易度選択画面.n現在の選択行] >= 0)
								{
									
									TJAPlayer3.stage選曲.t曲を選択する(this.act難易度選択画面.n現在の選択行);
									TJAPlayer3.Skin.sound曲決定音.t再生する();
								}
							}
							else if (this.act難易度選択画面.n現在の選択行 == -1)
							{
								TJAPlayer3.Skin.sound決定音.t再生する();
								this.actQuickConfig.tActivatePopupMenu(E楽器パート.TAIKO);
							}
							else if (this.act難易度選択画面.n現在の選択行 == -2)
							{
								TJAPlayer3.stage選曲.t難易度選択画面から戻る();
                    ...
                    else if (!this.actSortSongs.bIsActivePopupMenu && !this.actQuickConfig.bIsActivePopupMenu)
					{
						if (TJAPlayer3.Pad.b押された(E楽器パート.DRUMS, Eパッド.RBlue))
						{
							this.act難易度選択画面.t次に移動2();
							TJAPlayer3.Skin.sound変更音.t再生する();
						}
						else if (TJAPlayer3.Pad.b押された(E楽器パート.DRUMS, Eパッド.LBlue))
						{
							this.act難易度選択画面.t前に移動2();
							TJAPlayer3.Skin.sound変更音.t再生する();
						}
						else if (TJAPlayer3.Pad.b押されたDGB(Eパッド.Decide) || (TJAPlayer3.ConfigIni.bEnterがキー割り当てのどこにも使用されていない && TJAPlayer3.Input管理.Keyboard.bキーが押された((int)SlimDX.DirectInput.Key.Return)) || (TJAPlayer3.Pad.b押されたDGB(Eパッド.LRed) || TJAPlayer3.Pad.b押されたDGB(Eパッド.RRed)))
						{
							if (this.act難易度選択画面.n現在の選択行2 == 1)
							{
								TJAPlayer3.stage選曲.t難易度選択画面から戻る();
								ct難易度選択用 = new CCounter(0, 0, 3, TJAPlayer3.Timer);
							}
							else if (this.act難易度選択画面.n現在の選択行2 == -1)
							{
								TJAPlayer3.Skin.sound決定音.t再生する();
								this.actQuickConfig.tActivatePopupMenu(E楽器パート.TAIKO);
							}
							else if (this.act難易度選択画面.n現在の選択行2 == 0)
							{
								TJAPlayer3.Skin.sound曲決定音.t再生する();
								TJAPlayer3.stage選曲.t曲を選択する();
							}
