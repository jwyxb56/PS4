...
//			this.On非活性化();
		...
		public override int On進行描画()
		{
			...
			{
				tenkai = -66 + this.tenkai.n現在の値;
			}

			int tenkai1 = 0;

			if (this.tenkai.n現在の値 >= 100)
			{
				tenkai1 = -100 + this.tenkai.n現在の値;
			}
				...
				while( ( n現在時刻 - this.nスクロールタイマ ) >= nアニメ間隔 )
				{
					...
					if( this.n現在のスクロールカウンタ >= 100 )		// １行＝100カウント。
					{
                        ...
                        for ( int f = 0; f < (int)Difficulty.Total; f++ )
                        {
							...
							if (song.arスコア[f] != null)
								this.stバー情報[index].nScore = song.arスコア[f].譜面情報.nハイスコア;
						}
						// stバー情報[] の内容を1行ずつずらす。
						...
						for( int i = 0; i < 13; i++ )
						{
                            ...
                            this.stバー情報[ i ].ttkタイトル = this.ttk曲名テクスチャを生成する( this.stバー情報[ i ].strタイトル文字列, TJAPlayer3.Skin.SongSelect_NowSelect_BackColor, TJAPlayer3.Skin.SongSelect_NowSelect_ForeColor);
			...
			if( !this.b登場アニメ全部完了 )
			{
				if (this.n現在のスクロールカウンタ == 0)
				{
					...
					TJAPlayer3.Tx.SongSelect_Bar_Center[frameBoxIndex]?.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 960, TJAPlayer3.Skin.SongSelect_Overall_Y + 397);
				}
				{ 
                        ...
                        case C曲リストノード.Eノード種別.SCORE:
                            {
                                if (TJAPlayer3.Tx.SongSelect_Frame_Score != null)
                                {
                                    ...
                                    if (TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Tower && TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Dan)
                                    {
										...
										{
											if (TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[i] >= 0 && this.tenkai.n現在の値 >= 100)
											{
												// レベルが0以上
												...
												{
													// エディット
													TJAPlayer3.Tx.SongSelect_Frame_Score.t2D下中央基準描画(TJAPlayer3.app.Device, 693 + (3 * 182), TJAPlayer3.Skin.SongSelect_Overall_Y + 530, new Rectangle(182 * i, 0, 182, 87));
												}
												else if (i != 4)
												{
													TJAPlayer3.Tx.SongSelect_Frame_Score.t2D下中央基準描画(TJAPlayer3.app.Device, 693 + (i * 182), TJAPlayer3.Skin.SongSelect_Overall_Y + 530, new Rectangle(182 * i, 0, 182, 87));
												}
											}
											else if (this.tenkai.n現在の値 >= 100)
											{
												// レベルが0未満 = 譜面がないとみなす
								...
								#region[ 星 ]
								if (TJAPlayer3.Tx.SongSelect_Level != null && this.tenkai.n現在の値 >= 100)
								{
									// 全難易度表示
									// 難易度がTower、Danではない
									if (TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Tower && TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Dan)
									{
										for (int i = 0; i < (int)Difficulty.Edit + 1; i++)
										{
											for (int n = 0; n < TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[i]; n++)
											{
												...
												if (i == 4 && TJAPlayer3.stage選曲.n現在選択中の曲の難易度 == 4)
												{
													TJAPlayer3.Tx.SongSelect_Level.t2D下中央基準描画(TJAPlayer3.app.Device, 748 + (3 * 182), TJAPlayer3.Skin.SongSelect_Overall_Y + 508, new Rectangle(35 * TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[i], 0, 35, 45));
												}
												
												if (i != 4)
												{
													TJAPlayer3.Tx.SongSelect_Level.t2D下中央基準描画(TJAPlayer3.app.Device, 748 + (i * 182), TJAPlayer3.Skin.SongSelect_Overall_Y + 508, new Rectangle(35 * TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[i], 0, 35, 45));
												}
				...
				for( int i = 0; i < 13; i++ )	// パネルは全13枚。
				{
					if( this.ct登場アニメ用[ i ].n現在の値 >= 0 )
					{
						...
						if( i == 5 )
						{
							...
							if (this.ttk選択している曲のサブタイトル != null)
							{
								ResolveTitleTexture(ttk選択している曲のサブタイトル).vc拡大縮小倍率.X = this.counter1.n現在の値 / 100f;
								ResolveTitleTexture(ttk選択している曲のサブタイトル).vc拡大縮小倍率.Y = this.counter1.n現在の値 / 100f;
								...
								tx選択している曲のサブタイトル.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 960, nサブタイY + 70 - tenkai2);
								if (this.ttk選択している曲の曲名 != null)
								{
									ResolveTitleTexture(this.ttk選択している曲の曲名).vc拡大縮小倍率.X = this.counter1.n現在の値 / 100f;
									ResolveTitleTexture(this.ttk選択している曲の曲名).vc拡大縮小倍率.Y = this.counter1.n現在の値 / 100f;
									ResolveTitleTexture(this.ttk選択している曲の曲名).t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, 960, TJAPlayer3.Skin.SongSelect_Overall_Y + 23 + 370);
								}
							}
							else
							{
			...
			else
			{
				...
				for( int i = 0; i < 13; i++ )	// パネルは全13枚。
				{
                    ...
                    int xAnime = this.ptバーの座標[ n見た目の行番号 ].X + ( (int) ( ( this.ptバーの座標[ n次のパネル番号 ].X - this.ptバーの座標[ n見た目の行番号 ].X ) * ( ( (double) Math.Abs( this.n現在のスクロールカウンタ ) ) / 100.0 ) ) );
					int y = this.ptバーの座標[ n見た目の行番号 ].Y + ( (int) ( ( this.ptバーの座標[ n次のパネル番号 ].Y - this.ptバーの座標[ n見た目の行番号 ].Y ) * ( ( (double) Math.Abs( this.n現在のスクロールカウンタ ) ) / 100.0 ) ) );
					double size = this.ptバーの拡大率[n見た目の行番号] + ((int)((this.ptバーの拡大率[n次のパネル番号] - this.ptバーの拡大率[n見た目の行番号]) * (((double)Math.Abs(this.n現在のスクロールカウンタ)) /100.0)));
					{
						...
						//-----------------
						if (n現在のスクロールカウンタ != 0)
							this.tジャンル別選択されていない曲バーの描画(xAnime - 20, TJAPlayer3.Skin.SongSelect_Overall_Y + y, this.stバー情報[nパネル番号].strジャンル, size);
						else if (n見た目の行番号 != 5)
							this.tジャンル別選択されていない曲バーの描画(xAnime - 20, TJAPlayer3.Skin.SongSelect_Overall_Y + y, this.stバー情報[nパネル番号].strジャンル, size);
						if (this.stバー情報[nパネル番号].b分岐[TJAPlayer3.stage選曲.n現在選択中の曲の難易度] == true && n見た目の行番号 != 5)
                            TJAPlayer3.Tx.SongSelect_Branch.t2D描画(TJAPlayer3.app.Device, xAnime + 66, TJAPlayer3.Skin.SongSelect_Overall_Y - 40 + y);
						...
						ResolveTitleTexture(this.stバー情報[nパネル番号].ttkタイトル).vc拡大縮小倍率.X = (float)(size + 0f);
						ResolveTitleTexture(this.stバー情報[nパネル番号].ttkタイトル).vc拡大縮小倍率.Y = (float)(size + 0f);
					
						if (n現在のスクロールカウンタ != 0)
							ResolveTitleTexture(this.stバー情報[nパネル番号].ttkタイトル).t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, xAnime - 20, TJAPlayer3.Skin.SongSelect_Overall_Y + y);
						else if (n見た目の行番号 != 5)
							ResolveTitleTexture(this.stバー情報[nパネル番号].ttkタイトル).t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, xAnime - 20, TJAPlayer3.Skin.SongSelect_Overall_Y + y);
						...
						if ( this.stバー情報[ nパネル番号 ].ar難易度 != null )
                        {
                            ...
                            if( this.stバー情報[ nパネル番号 ].ar難易度[ TJAPlayer3.stage選曲.n現在選択中の曲の難易度 ].ToString().Length == 2 )
                                nX補正 = -6;
                            this.t小文字表示( xAnime + 65, 559, this.stバー情報[ nパネル番号 ].ar難易度[ TJAPlayer3.stage選曲.n現在選択中の曲の難易度 ].ToString() + nX補正);
                        }
						//-----------------
                        ...
                        case C曲リストノード.Eノード種別.SCORE:
                            {
                                if (TJAPlayer3.Tx.SongSelect_Frame_Score != null)
                                {
                                    ...
                                    {
                                        if (TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[TJAPlayer3.stage選曲.n現在選択中の曲の難易度] >= 0 && this.tenkai.n現在の値 >= 100)
                                        {
                                            // 譜面がありますね
                                            TJAPlayer3.Tx.SongSelect_Frame_Score.color4 = new Color4(1f, 1f, 1f);
                                            TJAPlayer3.Tx.SongSelect_Frame_Score.t2D下中央基準描画(TJAPlayer3.app.Device, 960, TJAPlayer3.Skin.SongSelect_Overall_Y + 463, new Rectangle(0, 360 + (360 * (TJAPlayer3.stage選曲.n現在選択中の曲の難易度 - (int)Difficulty.Tower)), TJAPlayer3.Tx.SongSelect_Frame_Score.szテクスチャサイズ.Width, 360));
                                        }
                                        else if (this.tenkai.n現在の値 >= 100)
										{
                                            // ないですね
		...
		{ new Point( -526, 130), new Point( -280, 151 ), new Point( 80 , 164 ), new Point( 348, 176 ), new Point( 634, 182 ),
		  new Point( 960, 327 ),
		  new Point( 1326, 182 ), new Point( 1612, 176 ), new Point( 1880, 164 ), new Point( 2100, 151 ), new Point(2400, 130 ), new Point( 3000, 130 ), new Point( 3000, 130 ) };

		private double[] ptバーの拡大率 = new double[] { 0.52 ,0.61, 0.66, 0.7, 0.75, 
			1,
			0.75, 0.7, 0.66, 0.61,0.52, 0.44, 1, 1 };
  ...
  //      private CTexture txバー中央;
		...
		private void tバーの初期化()
		{
			...
			for( int i = 0; i < 13; i++ )
			{
			    ...
			    for( int f = 0; f < (int)Difficulty.Total; f++ )
                {
					...
					if (song.arスコア[f] != null)
						this.stバー情報[i].nScore = song.arスコア[ f ].譜面情報.nハイスコア;
				}
				for ( int j = 0; j < 3; j++ )
		...
		private void tジャンル別選択されていない曲バーの描画( int x, int y, string strジャンル , double size)
		{
			...
			TJAPlayer3.Tx.SongSelect_Bar_Genre[barGenreIndex].vc拡大縮小倍率.X = (float)(size + 0f);
			TJAPlayer3.Tx.SongSelect_Bar_Genre[barGenreIndex].vc拡大縮小倍率.Y = (float)(size + 0f);
			TJAPlayer3.Tx.SongSelect_Bar_Genre[barGenreIndex]?.t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, x, y);
			int toumei = 0;
			if (TJAPlayer3.stage選曲.ct難易度選択用.n現在の値 <= 100)
			{
				toumei = 100 - TJAPlayer3.stage選曲.ct難易度選択用.n現在の値;
			}

		    //TJAPlayer3.Tx.SongSelect_Bar_Genre[barGenreIndex]?.t2D拡大率考慮中央基準描画( TJAPlayer3.app.Device, x, y );
			//TJAPlayer3.Tx.SongSelect_Bar_Genre[barGenreIndex].Opacity = ((toumei * 0xff) / 100);
			if (!TJAPlayer3.stage選曲.n)
			{
				TJAPlayer3.Tx.SongSelect_Bar_Genre[barGenreIndex].Opacity = ((100 * 0xff) / 100);
        ...
        {
            return new TitleTextureKey(str文字, pfMusicName, forecolor, backcolor, 390);
        }
